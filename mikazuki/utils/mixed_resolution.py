...
def build_mixed_resolution_plan(config: dict, *, training_type: str) -> MixedResolutionPlan:
    ...
    for stage_side, ratio_percent in stage_ratios:
        ...
        batch_size = max(1, int(math.floor(base_batch_size * resolution_area(base_resolution) / resolution_area(resolution))))
        per_epoch_batches = math.ceil(total_samples / batch_size)
        steps_per_epoch = math.ceil(per_epoch_batches / world_size / base_gradient_accumulation_steps)

        raw_epochs = int(math.ceil(base_epochs * (ratio_percent / 100.0) * (batch_size / base_batch_size)))
        actual_epochs = round_up_to_multiple(max(1, raw_epochs), alignment_epochs)
...
def build_mixed_resolution_summary_text(plan: MixedResolutionPlan) -> str:
    ...
    lines = [
        ...
        f"- 基准 batch: {plan.base_batch_size}",
    ...
    for phase in plan.phases:
        lines.append(
            f"- {phase.label}: 占比 {phase.ratio_percent:.2f}% | batch {phase.train_batch_size} | "
            f"steps/epoch {phase.steps_per_epoch} | raw_epoch {phase.raw_epochs} | "
            f"actual_epoch {phase.actual_epochs} | phase_steps {phase.phase_steps} | "
...
def build_phase_run_configs(config: dict, *, training_type: str) -> tuple[MixedResolutionPlan, list[dict]]:
    ...
    for phase in plan.phases:
        ...
        phase_config["train_batch_size"] = phase.train_batch_size
        ...
        phase_config["training_comment"] = merge_training_comment(
            ...
            f"mixed-resolution phase {phase.label} ratio={phase.ratio_percent:.2f}% target_max_steps={phase.cumulative_steps}",
        )
        if resume_path:
