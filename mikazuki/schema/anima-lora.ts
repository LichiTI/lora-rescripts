Schema.intersect([
    ...
    Schema.intersect([
        Schema.object({
            lora_type: Schema.union(["lora", "lokr"]).default("lora").description("适配器类型。LoRA 更轻量；LoKr 更适合复杂画风或角色"),
        ...
        Schema.union([
            ...
            Schema.object({
                ...
                lokr_factor: Schema.number().min(-1).default(8).description("LoKr 分解因子，常用 4~16；填写 -1 表示无限分解"),
                conv_dim: Schema.number().default(4).description("LoKr 卷积分支 dim"),
                conv_alpha: Schema.number().default(1).description("LoKr 卷积分支 alpha"),
