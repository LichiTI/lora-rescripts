const STYLE_ID = "aesthetic-runtime-widget-style";
const ROOT_ID = "aesthetic-runtime-widget-root";
const OVERLAY_ID = "aesthetic-runtime-restart-overlay";
...
#${ROOT_ID} .aesthetic-runtime-card {
  padding: 16px;
}
#${ROOT_ID} .aesthetic-runtime-title {
  font-size: 16px;
  font-weight: 600;
  color: #213547;
  margin-bottom: 6px;
}
#${ROOT_ID} .aesthetic-runtime-desc {
  font-size: 13px;
  color: #5f6b7a;
  margin-bottom: 12px;
  line-height: 1.6;
}
#${ROOT_ID} .aesthetic-runtime-summary {
  font-size: 13px;
  color: #213547;
...
#${ROOT_ID} .aesthetic-runtime-actions {
  display: flex;
  gap: 10px;
  flex-wrap: wrap;
  margin-bottom: 12px;
}
#${ROOT_ID} .aesthetic-runtime-actions button {
...
#${ROOT_ID} .aesthetic-runtime-actions button.secondary {
  background: #ffffff;
  color: #409eff;
}
#${ROOT_ID} .aesthetic-runtime-actions button[disabled] {
  opacity: 0.55;
  cursor: not-allowed;
}
#${ROOT_ID} .aesthetic-runtime-log {
  background: #0f172a;
  color: #cbd5e1;
  border-radius: 10px;
  padding: 12px;
...
#${OVERLAY_ID} .aesthetic-runtime-overlay-card {
  width: min(520px, 100%);
  background: #ffffff;
  border-radius: 16px;
  border: 1px solid #d9e3f0;
...
#${OVERLAY_ID} .aesthetic-runtime-overlay-title {
  font-size: 22px;
  font-weight: 600;
  color: #213547;
  margin-bottom: 10px;
}
#${OVERLAY_ID} .aesthetic-runtime-overlay-desc {
  font-size: 14px;
  color: #5f6b7a;
  line-height: 1.7;
  white-space: pre-wrap;
...
function showRestartOverlay(initialText) {
  ...
  overlay.innerHTML = `
    <div class="aesthetic-runtime-overlay-card">
      <div class="aesthetic-runtime-overlay-title">后端重启中</div>
      <div class="aesthetic-runtime-overlay-desc">${initialText || "正在请求重启后端，请稍候。"}</div>
    </div>
  `;
...
function renderWidgetState(root, payload, actions) {
  ...
  root.querySelector(".aesthetic-runtime-summary").textContent = formatSummary(payload);
  root.querySelector(".aesthetic-runtime-log").textContent = formatLogs(payload);
...
function createWidgetShell() {
  ...
  root.id = ROOT_ID;
  root.innerHTML = `
    <div class="aesthetic-runtime-card">
      <div class="aesthetic-runtime-title">美学评分运行依赖</div>
      <div class="aesthetic-runtime-desc">这里可以按需安装当前运行环境缺失的美学评分依赖。安装完成后，重启后端以重新加载新模块。</div>
      <div class="aesthetic-runtime-summary">正在读取美学评分运行状态...</div>
      <div class="aesthetic-runtime-actions">
      ...
      <pre class="aesthetic-runtime-log">正在读取安装日志...</pre>
    </div>
  `;
  return root;
}

export function mountAestheticRuntimeWidget() {
  ...
  const fetchRuntimeStatus = async () => {
    const response = await apiGet("/api/aesthetic/runtime_status");
    if (response.status !== "success") {
      throw new Error(response.message || "读取美学评分运行状态失败。");
    }
  ...
  const refreshWidget = async () => {
    ...
    try {
      ...
      root.querySelector(".aesthetic-runtime-summary").textContent = String(error);
    } finally {
      schedulePoll();
    }
  };
  ...
  const handleInstall = async () => {
    try {
      const response = await apiPost("/api/aesthetic/install_dependencies");
      if (response.status !== "success") {
        throw new Error(response.message || "美学评分依赖安装任务启动失败。");
      }
      await refreshWidget();
    } catch (error) {
  ...
  const waitForBackendReady = async (overlay) => {
    ...
    try {
      ...
      if (response.status === "success") {
        ...
        const detailNode = overlay.querySelector(".aesthetic-runtime-overlay-desc");
        if (detailNode) {
