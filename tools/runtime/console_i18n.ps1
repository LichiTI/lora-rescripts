...
$script:ConsoleTextTable = @{
    en = @{
        ...
        launcher_profile_blackwell_sage_label = 'Blackwell SageAttention (Experimental)'
        launcher_profile_blackwell_sage_description = 'Use the dedicated Blackwell SageAttention runtime and prefer SageAttention by default.'
        ...
        runtime_sageattention_blackwell = 'Blackwell SageAttention'
        runtime_sageattention_blackwell_python = 'Blackwell SageAttention experimental Python'
        ...
        exclusive_runtime_preference_error = 'Only one dedicated runtime can be preferred at a time. Clear MIKAZUKI_PREFERRED_RUNTIME or choose blackwell / sageattention / sageattention2 / sageattention-blackwell.'
...
"@
        ...
        startup_mode_sageattention2 = 'SageAttention2 startup mode enabled. This runtime prepares the dedicated SageAttention 2.x environment; enable sageattn manually on supported routes.'
        startup_mode_blackwell_sageattention = 'Blackwell SageAttention startup mode enabled. This runtime prepares the dedicated Blackwell SageAttention environment; enable sageattn manually on supported routes.'
    ...
    zh = @{
        ...
        launcher_profile_blackwell_sage_label = 'Blackwell SageAttention（实验性）'
        launcher_profile_blackwell_sage_description = '使用 Blackwell SageAttention 专用运行时，并默认优先使用 SageAttention。'
        ...
        runtime_sageattention_blackwell = 'Blackwell SageAttention'
        runtime_sageattention_blackwell_python = 'Blackwell SageAttention 实验运行时 Python'
        runtime_portable = '便携版 Python'
        runtime_portable_python = '便携版 Python'
        runtime_venv = '项目虚拟环境'
        runtime_venv_python = '项目虚拟环境'
        runtime_main = '主运行时'
        exclusive_runtime_preference_error = '同一时间只能指定一个专用运行时。请清理 MIKAZUKI_PREFERRED_RUNTIME，或在 blackwell / sageattention / sageattention2 / sageattention-blackwell 中只保留一个。'
...
"@
        ...
        startup_mode_sageattention2 = '已启用 SageAttention2 启动模式。这个运行时只负责准备 SageAttention 2.x 专用环境；支持的训练路线仍需手动启用 sageattn。'
        startup_mode_blackwell_sageattention = '已启用 Blackwell SageAttention 启动模式。这个运行时只负责准备 Blackwell SageAttention 专用环境；支持的训练路线仍需手动启用 sageattn。'
    ...
    ja = @{
        ...
        launcher_profile_blackwell_sage_label = 'Blackwell SageAttention（実験的）'
        launcher_profile_blackwell_sage_description = 'Blackwell SageAttention 専用ランタイムを使用し、既定で SageAttention を優先します。'
        ...
        runtime_sageattention_blackwell = 'Blackwell SageAttention'
        runtime_sageattention_blackwell_python = 'Blackwell SageAttention 実験ランタイム Python'
        ...
        exclusive_runtime_preference_error = '専用ランタイムは同時に 1 つだけ指定できます。MIKAZUKI_PREFERRED_RUNTIME を消去するか、blackwell / sageattention / sageattention2 / sageattention-blackwell のどれか 1 つだけを指定してください。'
...
"@
        ...
        startup_mode_sageattention2 = 'SageAttention2 起動モードが有効です。このランタイムは SageAttention 2.x 専用環境の準備のみを行います。対応ルートでは sageattn を手動で有効化してください。'
        startup_mode_blackwell_sageattention = 'Blackwell SageAttention 起動モードが有効です。このランタイムは Blackwell SageAttention 専用環境の準備のみを行います。対応ルートでは sageattn を手動で有効化してください。'
...
function Get-ConsoleRuntimeDisplayName {
    ...
    switch ($RuntimeName) {
        ...
        'sageattention-blackwell' { return Get-ConsoleText -Key "runtime_sageattention_blackwell$suffix" }
