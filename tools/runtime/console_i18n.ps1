...
$script:ConsoleTextTable = @{
    en = @{
        ...
        exclusive_runtime_preference_error = 'Only one dedicated runtime can be preferred at a time. Clear MIKAZUKI_PREFERRED_RUNTIME or choose flashattention / blackwell / sageattention.'
...
"@
    ...
    zh = @{
        ...
        runtime_main = '主运行时'
        exclusive_runtime_preference_error = '同一时间只能指定一个专用运行时。请清理 MIKAZUKI_PREFERRED_RUNTIME，或在 flashattention / blackwell / sageattention 中只保留一个。'
...
"@
    ...
    ja = @{
        ...
        exclusive_runtime_preference_error = '専用ランタイムは同時に 1 つだけ指定できます。MIKAZUKI_PREFERRED_RUNTIME を消去するか、flashattention / blackwell / sageattention のどれか 1 つだけを指定してください。'
