...
def _resolve_jtp3_model_ids(
    ...
    resolved_model = env_model or str(model_id).strip()
    resolved_fallback = _norm_opt_str(fallback_model_id)
    if env_fallback_raw is not None:
...
def _default_model_cache_root() -> Path:
    ...
    return (Path(__file__).resolve().parents[4] / "model" / "_models").resolve()
...
def _candidate_redrocket_repo_dirs(model_id: str) -> list[Path]:
    ...
    if mid in {"redrocket/jtp-3", "hf-hub:redrocket/jtp-3"}:
        out.append((_default_model_cache_root() / "repos" / "RedRocket__JTP-3").resolve())
    uniq: list[Path] = []
    seen = set()
    for x in out:
        k = str(x).lower()
...
class JTP3FeatureExtractor(nn.Module):
    ...
    def __init__(
        ...
        if not self._is_classifier_backend and hasattr(self.model, "vision_model"):
